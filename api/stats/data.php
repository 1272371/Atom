	...
	*/
	...
	if($_SERVER['REQUEST_METHOD']=='GET')
	{
		...
		elseif(isset($_GET['courses']) && $_GET['courses']=='courses')
		{
			...
			$query="SELECT mark.mark_total,course.course_name,mark.user_id FROM mark,course WHERE mark.user_id=:student_nr AND course.course_id=mark.course_id";
			$stmt = $pdo->prepare($query);
			$stmt->execute([':student_nr'=>$student]);
			...
			while($row = $stmt->fetch(PDO::FETCH_ASSOC))
			{
				$response .= '{"student":"'.htmlspecialchars($row['user_id']).'","mark_total":"'.htmlspecialchars($row['mark_total']).'","course_name":"'.htmlspecialchars($row['course_name']).'"},';
			}
			$response = substr($response,0,strlen($response)-1);
			$response .= ']';
